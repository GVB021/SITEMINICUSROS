...
type Section =
  ...
  | "sessions" | "takes" | "logs" | "integrations" | "db_monitoring";
...
const NAV: { id: Section; label: string; icon: React.ElementType }[] = [
  ...
  { id: "db_monitoring", label: "Monitoramento DB", icon: Database },
  { id: "logs", label: "Logs de Auditoria", icon: ClipboardList },
  { id: "integrations", label: "API e Integracoes", icon: KeyRound },
];

const ROLES = ["MASTER", "ADMINISTRADOR", "DIRETOR", "DUBLADOR", "ALUNO"];
...
function roleBadgeVariant(role: string): "destructive" | "default" | "secondary" | "outline" {
  if (role === "MASTER") return "destructive";
...
function DBMonitoringSection() {
  const { data: stats, isLoading } = useQuery({
    queryKey: ["/api/admin/db-stats"],
    queryFn: () => authFetch("/api/admin/db-stats") as Promise<any>,
    refetchInterval: 5000,
  });

  if (isLoading) return <div className="p-6 text-sm text-muted-foreground">Carregando métricas...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total de Queries" value={stats?.totalQueries ?? 0} icon={Activity} color="text-blue-400" />
        <StatCard label="Queries Lentas (>100ms)" value={stats?.slowQueries ?? 0} icon={AlertCircle} color="text-amber-400" />
        <StatCard label="Erros de DB" value={stats?.errors ?? 0} icon={XCircle} color="text-rose-400" />
        <StatCard label="Tempo Médio" value={`${stats?.avgQueryTimeMs ?? 0}ms`} icon={RefreshCw} color="text-emerald-400" />
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        <div className="vhub-card overflow-hidden">
          <div className="vhub-card-header flex items-center gap-2 text-sm font-medium">
            <Database className="h-4 w-4" /> Connection Pool
          </div>
          <div className="vhub-card-body space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Conexões Totais</span>
              <span className="font-mono font-bold">{stats?.pool?.totalCount ?? 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Conexões Inativas</span>
              <span className="font-mono font-bold text-emerald-400">{stats?.pool?.idleCount ?? 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Queries em Fila</span>
              <span className="font-mono font-bold text-amber-400">{stats?.pool?.waitingCount ?? 0}</span>
            </div>
          </div>
        </div>
        ...
        <div className="vhub-card overflow-hidden">
          <div className="vhub-card-header flex items-center gap-2 text-sm font-medium">
            <ShieldAlert className="h-4 w-4" /> Saúde do Sistema
          </div>
          <div className="vhub-card-body flex flex-col items-center justify-center py-6">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-3 ${
              (stats?.errors > 0 || stats?.slowQueries > 5) ? "bg-rose-500/20 text-rose-500" : "bg-emerald-500/20 text-emerald-500"
            }`}>
              { (stats?.errors > 0 || stats?.slowQueries > 5) ? <AlertCircle className="h-8 w-8" /> : <BadgeCheck className="h-8 w-8" /> }
            </div>
            <p className="text-lg font-bold">
              {(stats?.errors > 0) ? "Atenção Necessária" : "DB Saudável"}
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              Último reset: {new Date(stats?.lastReset).toLocaleString()}
            </p>
          </div>
        </div>
      </div>
...
export default function Admin() {
  ...
  if (!isMaster && !isAdmin) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background space-y-4">
        ...
        <p className="text-muted-foreground text-sm">Apenas administradores podem acessar este painel.</p>
  ...
  const sectionMap: Record<Section, React.ReactNode> = {
    ...
    takes: <TakesSection />,
    db_monitoring: <DBMonitoringSection />,
    logs: <LogsSection />,
  ...
  const filteredNav = NAV.filter(item => {
    ...
    // For ADMINISTRADOR, hide platform-wide tabs
    return !["studios", "users", "logs", "integrations"].includes(item.id);
  });
