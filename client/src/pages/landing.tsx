      ...
      {canEditLandingText && isEditorOpen && (
        <div className="fixed right-4 bottom-16 z-30 w-[min(92vw,460px)] rounded-2xl border border-border bg-background/95 backdrop-blur p-4 shadow-2xl space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input className="rounded-md border border-border bg-background px-2 h-9 text-xs" value={textConfig.navHubDub} onChange={(e) => setTextConfig((c) => ({ ...c, navHubDub: cleanSingleLine(e.target.value, c.navHubDub, 16) }))} placeholder="HUBDUB" />
            <input className="rounded-md border border-border bg-background px-2 h-9 text-xs" value={textConfig.authEnter} onChange={(e) => setTextConfig((c) => ({ ...c, authEnter: cleanSingleLine(e.target.value, c.authEnter, 16) }))} placeholder="Entrar" />
            <input className="rounded-md border border-border bg-background px-2 h-9 text-xs" value={textConfig.authPanel} onChange={(e) => setTextConfig((c) => ({ ...c, authPanel: cleanSingleLine(e.target.value, c.authPanel, 16) }))} placeholder="Painel" />
          </div>
          <textarea
