...
function normalizeWsRole(role: string | undefined) {
  return String(role || "").trim().toLowerCase();
}

export function isUserPresentInVideoSession(sessionId: string, userId: string): boolean {
  const room = rooms.get(String(sessionId || ""));
  if (!room || !userId) return false;
  let found = false;
  room.forEach((ws) => {
    if (found) return;
    if (String(ws.userId || "") === String(userId)) found = true;
  });
  return found;
}

export function isUserActiveDirectorInVideoSession(sessionId: string, userId: string): boolean {
  const room = rooms.get(String(sessionId || ""));
  if (!room || !userId) return false;
  let found = false;
  room.forEach((ws) => {
    if (found) return;
    if (String(ws.userId || "") !== String(userId)) return;
    const role = normalizeWsRole(ws.role);
    if (role === "diretor" || role === "director") found = true;
  });
  return found;
}

export function hasActiveDirectorInVideoSession(sessionId: string): boolean {
  const room = rooms.get(String(sessionId || ""));
